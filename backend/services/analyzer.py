...
- skip 판단 기준 (아래 중 하나라도 해당하면 skip=true):
  · 인사말, 자기소개, 아웃트로, 구독/좋아요/알림 요청
  · "오늘은 ~를 알아볼게요" 같은 내용 예고만 하고 실제 정보 없는 도입부
  · 다음 주제로 넘어가는 연결 멘트·전환 구간
  · 영상 주제와 직접 관련 없는 잡담·사담
  · 단독으로 봤을 때 시청자에게 실질적 정보나 가치를 주지 못하는 구간
  skip=false: 시청자가 혼자 봐도 영상 주제(인테리어·요리·운동 등)에 대한 실질적 정보·팁·방법·결과·비교를 얻을 수 있는 구간
...
- skip 판단 기준 (아래 중 하나라도 해당하면 skip=true):
  · 인사말, 자기소개, 아웃트로, 구독/좋아요/알림 요청
  · 내용 예고만 하고 실제 정보 없는 도입부, 다음 주제 연결 멘트
  · 영상 주제와 직접 관련 없는 잡담·사담
  · 단독으로 봐도 실질적 정보·가치를 주지 못하는 구간
  skip=false: 시청자가 혼자 봐도 실질적 정보·팁·방법·결과를 얻을 수 있는 구간
- recommended: skip=false인 주제 중 특히 임팩트 있어 쇼츠로 만들면 좋은 것만 true
⚠️ ranges의 start/end는 자막의 숫자(초)를 그대로 사용
